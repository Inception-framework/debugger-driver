...
void USBDevice::quit (void) {

								this->jtag_libusb_close();
}

bool USBDevice::string_descriptor_equal(libusb_device_handle *device, uint8_t str_index,
																																								const char *string)
{
								int retval;
								bool matched;
								...
								if (retval < 0) {
																ALERT("Device","libusb_get_string_descriptor_ascii() failed with %d", retval);
																return false;
								}
																																	...
																																	struct libusb_device_handle **out)
{
								...
								for (idx = 0; idx < cnt; idx++) {
																...
																if (errCode) {
																								ALERT("Device","libusb_open() failed with %s", libusb_error_name(errCode));
																								continue;
																}
...
void USBDevice::init (void) {
								...
								if( retval == 0 ) {
																...
																case LIBUSB_ERROR_NOT_FOUND:
																								ALERT("Device","The interface requested by avatar driver does not exist");
																								break;

																case LIBUSB_ERROR_BUSY:
																								ALERT("Device","A driver different than avatar driver has claimed the interface");
																								break;

																case LIBUSB_ERROR_NO_DEVICE:
																								ALERT("Device","Avatar driver detected that the device has been disconnected");
																								break;
...
void USBDevice::write () {
								...
								do {
																																																			...
																																																			1) ) != 0 )
																{
																								...
																								case LIBUSB_ERROR_NO_DEVICE:
																																...
																																break;
																								default:
																																ALERT("Device","Avatar driver failed to write : error code %d ", retval);
																																break;
																								}
																								sleep(2);
																								attempt++;
								...
								if( attempt >= 6) {
																ALERT("Device","Avatar driver failed to communicate with device ... 6 attempts");
																//exit(-1);
								}
